...
library(readr); library(dplyr); library(lubridate); library(caret); library(AppliedPredictiveModeling); library(doParallel); library(randomForest) # Load libraries
...
rawFolder <- file.path(getwd(), "raw"); trainingFile <- file.path(rawFolder, "training.csv"); evaluationFile <- file.path(rawFolder, "evaluation.csv") # Assign variables for raw folder and source file names
...
```{r importance, cache=TRUE, dependson='cleandata'}
...
mySettings <- list(superpose.symbol = list(alpha = rep(0.1, 5),col = myColors, pch = rep(16,5))) # Modify theme superpose symbol settings

randSample <- sort(sample.int(dim(training)[1],1000)) # Use a smaller random sample to make easier to spot the class clouds

featurePlot(x = training[, varsImportance],  y = training$classe, plot = "ellipse", auto.key = list(columns = 5), par.settings = mySettings) # And finally generate the plot
```
...
From the previous analysis we identified the top-3 best performing models, now we're going to combine them or create an ensemble model.
